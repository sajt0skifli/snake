...
const int w=500,h=500,
          blocksize=20, //rács négyzet mérete (pixelben) - textúrák 20 esetére tervezve, nem érdemes megváltoztatni
          falvastag=2,  //pályaszéli oldalfal vastagsága (blockban)
          beltav=4,     //belso fal oldalfaltól való távolsága (blockban)
          almatime=7;   //alma létrehozásának gyakorisága (lépésben) (pl.: megevéstől számított 7. lépésnél)
...
int main()
{
    ...
    hatter.open(w,h);                       //
    racs.hatter(hatter,fu,fal);             //   Rács és háttér létrehozása
    gout << stamp(hatter,0,0);              //
